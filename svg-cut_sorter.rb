...
def is_in_rect_svgline(line, x1, y1, x2, y2)
    if line =~ /["]translate\(/
        ...
        # p ["translate",x,y]
        return [is_in_rect?(x, y, x1, y1, x2, y2), [ x,y] ]
    elsif line =~ /<circle/
       cx, cy = line.match(/cx="([^"]+)"\s+cy="([^"]+)"/)&.captures&.map{|z| z.to_f}
      #  p ["circle",cx,cy]
       return [is_in_rect?(cx, cy, x1, y1, x2, y2), [cx,cy]]
    elsif line =~ /<text/
        ...
        # p ["points",  points]
        return [points&.any?{|x,y| is_in_rect?(x, y, x1, y1, x2, y2)}, points]
    else
        [nil, []]
    end
end
...
def main(input_file_path, output_file_path, x1, y1, x2, y2,dx)
    ...
    File.open(input_file_path, "r") do |inFile|
        ...
        inFile.each_line do |line|
          ...
          if line_progress == :atHeaders
            if line =~ /\sviewBox=/
              line.gsub!(/\sviewBox="[^"]*"/, " viewBox=\"#{x1} #{y1} #{x2 - x1} #{y2 - y1}\"")
            end
            if line =~ /\swidth=/
              line.gsub!(/\swidth="[^"]*"/, " width=\"#{x2 - x1}\"")
            end
            if line =~ /\sheight=/
              line.gsub!(/\sheight="[^"]*"/, " height=\"#{y2 - y1}\"")
            end
            svg_headBuf.push line
            line_progress = :atBody if line =~ /<\/defs>/
          elsif line_progress == :atBody
            whatAddr = is_in_rect_svgline(line, x1, y1, x2, y2)
            if whatAddr[0]
              svg_bodyBuf.push [whatAddr[1], line]
            elsif whatAddr[0].nil? && whatAddr[1][0].nil? && line =~ /<\/svg>/
    ...
    File.open(output_file_path, "w") do |outFile|
      ...
      svg_bodyBuf.each{|whatAddr, line| outFile.puts line}
...
if __FILE__ == $0
    ...
    x1, y1, x2, y2 = ARGV[1].to_f.to_i, ARGV[2].to_f.to_i, ARGV[3].to_f.to_i, ARGV[4].to_f.to_i
