...
module HighPrecisionMath
  ...
  # 例: -0.5 = -1 + 0.5, つまり [-1, 2]
  def self.continued_fraction(x, max_terms: 20)
    coefficients = []
    current = x # 絶対値を取らずにそのまま処理

    max_terms.times do |i|
      # 整数部分を取得 (floor)
      a = current.floor.to_i
      coefficients << a

      # 残差を計算
      remainder = current - BigDecimal(a.to_s)

      # 残差がゼロに近い場合は終了
      if remainder.abs < @@convergence_epsilon
        break
      end

      # 次の項を計算: 1/remainder
      begin
        current = BigDecimal('1') / remainder
      rescue ZeroDivisionError
        break
      end

      # 発散チェック（係数が異常に大きい場合）
      if current.abs > BigDecimal('10') ** 15
        coefficients << :inf
        break
      end
    end
  ...
  def self.continued_fraction_to_decimal(coeffs)
    return BigDecimal('0') if coeffs.empty?

    # 末尾から計算
    result = BigDecimal(coeffs.last.to_s)

    (coeffs.size - 2).downto(0) do |i|
      result = BigDecimal(coeffs[i].to_s) + (BigDecimal('1') / result)
    end
...
if __FILE__ == $0
  ...
  begin
    ...
    components.each_with_index do |vec, i|
      # 出力は5桁に丸めて指数表記
      puts "  Component #{i+1}: [#{vec.map{|x| x.round(12).to_s('E')}.join(', ')}]"
    end
