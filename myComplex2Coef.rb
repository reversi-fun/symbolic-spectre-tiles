...
class MyNumeric1Coef < MyNumericCoef
  ...
  def check_consistency()
    ...
    if ! result
     p  ["check_consistency failed",to_f , eval(to_s.gsub("√3", "#{Math.sqrt(3)}"))]
    end
    result
  end
...
class MyNumeric2Coef < MyNumericBase
  ...
  def -(other)
    if other.zero?
      ...
      p ["Invalid argument for operator " + self.class.name + " - " + other.class.name, self, other]
      raise ArgumentError.new("Invalid argument for operator " + self.class.name + " - " + other.class.name, self,other)
    end
  end
