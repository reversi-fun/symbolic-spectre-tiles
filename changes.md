...
**全般**:
- `puts` → `STDERR.puts` に全て変更（CSV出力以外）
- グループ不在時は警告を出すが処理は継続
- メモリ効率のため、CSVデータのコピーを作らない

**CSV逐次出力の詳細** (要件6):
- 探索ループ内で `CSV.open` ブロック内に配置
- 各形状が採用されるたびに即座に全14頂点をCSV出力
- `tail -f` コマンドで外部監視可能にすることが目的
- 出力形式: 12カラム（a0, a1, b0, b1, key, perp_x, perp_y, perp_sq, perp_x_common, perp_y_common, in_input, is_extra）
- グループ固有基底と共通基底の両方の射影値を出力
- 実装箇所: `my_spectre_coordinateAnalyzer.rb` L1119-1170

**デバッグ情報のstderr分離** (要件7):
- **全ての進捗・統計・警告メッセージを `STDERR.puts` で出力**
- 標準出力（stdout）はCSVデータ専用とする（リダイレクト対応）
- stderr出力対象:
  - データ読み込み・グループ化の進捗 (✅ データ読み込み完了...)
  - PCA分析・共通基底計算の統計 (📊 共通基底 固有値: ...)
  - 探索開始・進捗メッセージ (🚀 ハイブリッド探索を開始...)
  - デバッグ統計の詳細 ([DEBUG PERFORMANCE STATISTICS])
  - グループ不在警告 (⚠️ 警告: グループ ... が存在しません)
- 利点: `ruby script.rb input.csv > output.csv` のような使用が可能
