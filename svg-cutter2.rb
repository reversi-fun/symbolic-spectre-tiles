...
def is_in_rect_svgline(line, x1, y1, x2, y2)
    if line =~ /["]translate\(/
      ...
      #  p ["circle",cx,cy]
       return is_in_rect?(cx, cy, x1, y1, x2, y2)
    elsif line =~ /<text/
        ...
        # p ["points",  points]
        return points&.any?{|x,y| is_in_rect?(x, y, x1, y1, x2, y2)}
      else
        true
    end
end
    ...
    File.open(output_file_path, "w") do |outFile|
      File.open(input_file_path, "r") do |inFile|
        ...
        inFile.each_line do |line|
          ...
          in_defs_group = false if line =~ /<\/defs>/
          if in_defs_group
            outFile.puts line
          else
            if line =~ /\sviewBox=/
            ...
            if is_in_rect_svgline(line, x1, y1, x2, y2)
              outFile.puts line
            end
            if line =~ /<g/
